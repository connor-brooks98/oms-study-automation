...
Current source candidate: `146df998b88cd31c561cf8339b8c90abc95369ea`. The Rust path-resolution patch passed native Windows LPAC red/green tests, including canonical reopen and preserved read/write denial boundaries. The Codex temp-containment correction is independently reviewed. Connor disabled Smart App Control himself; fresh verification confirmed that change. A separate compiler include-path defect was reproduced and fixed. Serial compilation then passed the previously failing core/app-server crates, but final CLI compilation hit LLVM out-of-memory at both 8 and 9.5 GiB job caps. A proposed 10 GiB attempt stopped before launch when fresh headroom was insufficient. That 9.5 GiB run’s 109 memory checks, 18 Hub checks and independent cleanup/preservation passed. A native metadata comparison then verified that disabling cross-crate ThinLTO changes only the intended LTO fields, retaining other release settings. The independently reviewed rebuild reached its 90-minute bound without compiler errors; all 183 Hub and 1,260 memory checks passed, with cleanup independently verified. The first continuation passed core and app-server compilation, then also reached its normal deadline during TUI compilation without errors; all 183 Hub and 1,260 memory checks and independent cleanup/preservation passed. The second continuation succeeded: the custom executable is SHA256 `74d7706f403336b693853eb8582e0424d9f765ac6c2d98e408206bf03e5ecf4f` (293,978,624 bytes). All 144 Hub and 986 memory checks passed; peak job memory stayed below the 9.5 GiB cap, and independent cleanup/preservation passed. The exact LPAC version check passed with empty stderr and normal exit. Fixed initialize then failed before any response because `gethostname` panicked at `GetComputerNameExW`; the owned child was killed/reaped at its deadline. Cleanup and exact Hub/runtime preservation passed. A native API-only test confirmed LPAC access denial. The reviewed correction uses fallible Windows lookup only for the optional remote-control display name, preserving hostname-based policy and telemetry behavior. Three exact-helper local tests passed; the reviewed incremental Windows rebuild is running, and fresh startup acceptance remains pending. No retry or arg0 tests ran. Provider acceptance remains pending and generation remains closed. [Current fix and exact evidence](gpt-platform-handoffs/O-windows-canonicalize.md). Existing release archives below remain bound to original core code `9e54da4188abaa5694c57b0d59aaec7c4e216a4f`, not rebuilt packages for this candidate.
...
The successful build used the reviewed LTO-only profile adjustment, one Cargo job,
...
correction is implemented and reviewed; a new Windows build and fresh startup
checks are required before accepting it. The two Windows arg0 tests remain
