...
Current source candidate: `67c58784042601d958d1e86879f96c71a8365bda`. The documented lpacCom/registryRead correction is implemented and independently reviewed. Actual Windows compilation and exact two-capability suspended-token verification passed, but cmd still exited `0xC0000142` with empty output. A matched-symbol native loader trace identifies KERNELBASE.dll failure during DLL_PROCESS_ATTACH; the internal cause remains unknown. Read isolation and actual Codex startup remain unverified; generation stays closed. [Independent startup review](gpt-platform-handoffs/O-windows-startup-review.md). The LPAC probe is a separate network-free synthetic boundary test; its native acceptance status is recorded in [the LPAC handoff](gpt-platform-handoffs/O-windows-lpac.md). Existing release archives below remain bound to original core code `9e54da4188abaa5694c57b0d59aaec7c4e216a4f`; they are not rebuilt packages for this candidate. [Prior Windows artifact/router acceptance](gpt-platform-handoffs/O-windows-runtime.md).
