...
- Prior docs commit: `0c0f2b0b3013cad18a8f8b708f6df442d8dbb0e6` / tree
...
- `record_retrieval_run` stores source snapshot hash, evidence IDs, source revision IDs,
  provider request ID, prompt/schema/model versions, a required validation status code,
  and the expected evidence-link cardinality. Provider request IDs are bounded opaque
  IDs; validation outcomes are limited to `valid`, `invalid`, `rejected`, `insufficient`,
  or `error`. `retrieval_evidence`
...
1. In `src/oms_hub/models.py`, add central ORM classes with the same table/column contract
   ...
   composite FK; constrain `provider_request_id` to bounded opaque IDs within its
   `String(500)` column and constrain `validation_outcome` to the defined status set
   ...
   constraint enforced by the central repository boundary.
