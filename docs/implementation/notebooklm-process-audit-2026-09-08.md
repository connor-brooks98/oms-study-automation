...
| NLM-017 | P1 | Windows rollout cleanup found the same-root Python launcher but missed its base-interpreter children. Stopping the scheduled task left the old HTTP server running, preventing a safe checkout update. | Fixed in shared process discovery: queue the positively identified Python root before walking descendants. Existing ownership and pinned-process stop checks remain in place. | Native Windows mocked-process regression includes Python and executable launchers, descendants outside the deployment root, and unrelated processes that must be excluded. All 58 rollout checks passed on the NUC. |
| NLM-018 | P3 | A rollout test invoked `python3` through Git Bash, which resolved to the Windows Store alias on the NUC instead of the test interpreter. | Fixed the test harness to invoke the running Python executable through a scoped shell function. | All 58 rollout checks passed locally and on the NUC; no production PATH or interpreter configuration changed. |
...
- Full local Python suite: **3,324 passed, 1 skipped**, excluding the Windows Office marker. The isolated NUC checkout passed **645 affected application tests** in 1,048 seconds. The subsequent installer/test-harness correction did not change application source or dependencies; all **58 rollout checks** then passed natively on the NUC in 5 seconds and locally.
...
- The first stop attempt exposed NLM-017 before checkout mutation. The old server continued serving successfully throughout the initial HTTP monitor (zero failed polls). The recovery uses the existing guarded process-stop helper with corrected discovery before retrying the same authorized rollout.
