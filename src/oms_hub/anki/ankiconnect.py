...
class AnkiConnectClient:
    def __init__(
        ...
        *,
        ...
        if url != ANKI_CONNECT_URL:
            raise ValueError(
                f"AnkiConnect must use the loopback URL {ANKI_CONNECT_URL}"
            )
        self.url = url
    ...
    def notes_info(self, note_ids: Sequence[int]) -> list[dict[str, Any]]:
        ...
        if not isinstance(result, list) or not all(
            ...
            raise AnkiConnectProtocolError(
                "AnkiConnect returned invalid note information"
            )
    ...
    def add_tags(self, note_ids: Sequence[int], tags: Sequence[str]) -> None:
        ...
        if result is not None:
            raise AnkiConnectProtocolError(
                "AnkiConnect returned an invalid addTags result"
            )
    ...
    def sync(self) -> None:
        ...
        if result is not None:
            raise AnkiConnectProtocolError(
                "AnkiConnect returned an invalid sync result"
            )
