...
class SystemPromptDirectoryPicker:
    """Open a folder browser on the Study Hub device."""

    _SCRIPT = r"""
Add-Type -AssemblyName System.Windows.Forms
$dialog = New-Object System.Windows.Forms.FolderBrowserDialog
$dialog.Description = 'Choose the Anki AI Prompts directory'
...
"""
    ...
    def select_directory(self) -> Path | None:
        if os.name != "nt":
            raise RuntimeError(
                "Select Folder is available from the Study Hub running on the NUC"
            )
        try:
            result = subprocess.run(
                ["powershell.exe", "-NoProfile", "-STA", "-Command", self._SCRIPT],
                check=False,
                capture_output=True,
                text=True,
                timeout=300,
            ...
            raise RuntimeError("The NUC could not open its folder browser") from error
        if result.returncode != 0:
            raise RuntimeError("The NUC could not open its folder browser")
        selected = result.stdout.strip()
        if not selected:
            return None
        path = Path(selected).resolve()
        if not path.is_dir():
            raise RuntimeError("The selected prompt directory is no longer available")
        return path
