...
def test_conflicting_answer_entries_create_blocker() -> None:
    ...
    assert {item.code for item in drafts[0].diagnostics} == {"duplicate-supplied-answer"}
    assert {item.code for item in result.diagnostics} == {"unmatched-supplied-answer"}
...
def test_duplicate_question_ids_and_unmatched_entries_remain_blocked() -> None:
    ...
    assert any("unmatched supplied answer" in item.message for item in result.diagnostics)
...
def test_different_numbered_sets_do_not_fall_back_to_source_order() -> None:
    ...
    assert all(draft.correct_index is None for draft in drafts)
    assert any(
        "unmatched supplied answer" in diagnostic.message
        for diagnostic in result.diagnostics
    )


def test_different_nonnumeric_identifiers_do_not_fall_back_to_source_order() -> None:
    result = pair_supplied_answers((question("A", "First"),), (answer("B", 1),))
    drafts = result.drafts
