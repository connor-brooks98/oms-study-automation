...
from oms_anki_agent.snapshot import DeltaSnapshotPlanner, SnapshotVersions
...
def test_delta_planner_falls_back_without_safe_compatible_ledger(tmp_path: Path) -> None:
...
def test_delta_planner_falls_back_for_unsafe_window_or_count_drift(
    ...
    planner = DeltaSnapshotPlanner(ledger, maximum_window=timedelta(days=7))

    assert planner.plan(current_note_ids=[1], now=NOW, versions=VERSIONS).reason == (
        "unsafe_window"
    )
