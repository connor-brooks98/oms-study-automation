...
def test_outline_download_reuses_validation_and_forces_attachment(tmp_path):
    study_root = tmp_path / "study"
    path = study_root / "Neuro" / "Exam 1" / "Lecture Outlines" / "outline.pdf"
    path.parent.mkdir(parents=True)
    payload = OutlinePdfRenderer().render("Neuro Outline", "# Topic\nContent")
    ...
    job = app.state.generation_repository.queue(lecture_id, GenerationKind.OUTLINE)
    record = app.state.generation_repository.record_outline(
        lecture_id, job.id, path, hashlib.sha256(payload).hexdigest()
    )

    response = TestClient(app).get(f"/artifacts/outlines/{record.id}/download")
